// import preact
//{countries.map((countries, i) => <option value={countriess}>{countries}</option>)}
import { h, render, Component } from 'preact';

export default class ListCon extends Component {
	...
	render() {
		...
		return (
			<div>
				<form>
					<select value={this.props.countryValue} onChange={this.props.triggerChange}>
						{countries.map((countries, i) => <option value={countries}>{countries}</option>)}
					</select>
					<h1>{this.props.countryValue}</h1>
				</form>
			</div>
		);	
	}
